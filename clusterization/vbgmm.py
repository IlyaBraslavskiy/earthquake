...
class VBGMM:
    ...
    def UpdateParams(self):
        ...
        self.inv_W_k = - self.b_k.reshape(self.b_k.shape[0], 1, 1) * np.einsum("Bi, Bj -> Bij", self.m_k, self.m_k)
        ...
        for k in range(self.R_k.shape[1]):
            mix_cov[k,:,:] = np.sum(self.R_k[:,k].reshape(self.R_k.shape[0], 1, 1) * x_cov, axis=0)
    ...
    def UpdateR(self):
        ...
        for k in range(K):
            self.R_k[:,k] = st.multivariate_normal.logpdf(self.X, mean=self.m_k[k,:], cov=self.inv_W_k[k,:,:] / self.mu_k[k]) 
